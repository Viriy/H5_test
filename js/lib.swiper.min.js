...
* @Date: 2016-06-17 00:37:20

* @Last Modified by: wei jundong

* @Last Modified time: 2017-01-25 

*/

/*to load use reset-screen-size*/

//(function(document){var dcl=document.documentElement,wh;function setRootRem(){wh=dcl.clientHeight;dcl.style.fontSize=100*(wh/1334)+'px'}setRootRem();document.addEventListener('DOMContentLoaded',setRootRem,false);window.addEventListener('resize',setRootRem,false)})(document);
...
window.KeirTools={};(function(a){a.Timer=function(b){this.polyFills();this.dis=b["dis"]||0;this.loop=b["loop"]||0;this.count=0;this.callbackfun=b["callback"]||null;this.tid=null;this.pretime=0;this.ispause=false;this.param=b["param"]||null};a.Timer.prototype.start=function(){if(this.callbackfun==null){this.errors("回调函数不能为空");return}else{if(this.loop<0||this.dis<0){this.errors("请输入正确数值");return}}this.pretime=(new Date()).getTime();this.ispause=false;this.processing()};a.Timer.prototype.processing=function(){var b=this;if(this.loop>0&&this.count==this.loop){this.stop();return}if(this.ispause==true){return}if(this.dis>16.7){var c=(new Date()).getTime();if(c-this.pretime>this.dis){this.count++;this.callbackfun(this.count,this.param);this.pretime=c}}else{this.count++;this.callbackfun(this.count,this.param)}this.tid=window.requestAnimationFrame(function(){b.processing()})};a.Timer.prototype.stop=function(){if(this.tid!=null){this.ispause=true;window.cancelAnimationFrame(this.tid);this.count=0;this.tid=null}};a.Timer.prototype.pause=function(){if(this.tid!=null){this.ispause=true;window.cancelAnimationFrame(this.tid);this.tid=null}};a.Timer.prototype.errors=function(b){throw new Error(b)};a.Timer.prototype.polyFills=function(){var d=0;var e=["webkit","moz"];for(var b=0,c=e.length;b<c&&!window.requestAnimationFrame;++b){window.requestAnimationFrame=window[e[b]+"RequestAnimationFrame"];window.cancelAnimationFrame=window[e[b]+"CancelAnimationFrame"]||window[e[b]+"CancelRequestAnimationFrame"]}if(!window.requestAnimationFrame){window.requestAnimationFrame=function(j,g){var f=new Date().getTime();var h=Math.max(0,16.7-(f-d));var i=window.setTimeout(function(){j(f+h)},h);d=f+h;return i}}if(!window.cancelAnimationFrame){window.cancelAnimationFrame=function(f){clearTimeout(f)}}};a.Load51=function(c,b,d){this.Version="Load51_v3.0";this._isAutoLoad=c===undefined?true:c;this._isReady=false;this._isLoaded=false;this._num=0;this._targetNum=0.82;this._progressTime=null;this._loading=document.querySelector(".loading");this._progress=document.querySelector(".progress");this._progressNum=document.querySelector(".progress-num");this._readyCb=b||null;this._completeCb=d||null;this._onDomReady=this._readyHandler.bind(this);this._onWindowLoad=this._loadHandler.bind(this);this.init()};a.Load51.prototype={constructor:a.Load51,init:function(){if(this._isAutoLoad){this._addReadyEvent();this._addLoadEvent()}this._changeView()},_ready:function(){if(this._num<=this._targetNum){this._num+=0.01;this._progress.style.webkitTransform="scaleX("+this._num+")";this._progressNum.innerHTML=Math.round(this._num*100)+"%"}},_load:function(){this._num+=0.05;if(this._num>=1){this._removeReadyEvent();this._removeLoadEvent();this._removeTimer(this._progressTime);this._loading.style.display="none";typeof this._completeCb==="function"&&this._completeCb()}this._progress.style.webkitTransform="scaleX("+(this._num>1?1:this._num)+")";this._progressNum.innerHTML=(Math.round(this._num*100)>100?100:Math.round(this._num*100))+"%"},onReadyStart:function(b){this._readyCb=b?b:this._readyCb;if(!this._isAutoLoad){this._readyHandler()}else{if(this._isReady){console.warn("DOMContentLoaded has already done it,if you need to manually call, please set the isAutoLoad to false")}}},onComplete:function(b){this._completeCb=b?b:this._completeCb;if(!this._isAutoLoad){this._loadHandler()}else{if(this._isLoaded){console.warn("The load event has been executed,if you need to manually call, please set the isAutoLoad to false")}}},_readyHandler:function(){this._isReady=true;typeof this._readyCb==="function"&&this._readyCb()},_loadHandler:function(){this._isLoaded=true},_changeView:function(){var b=this;this._progressTime=new a.Timer({dis:100,callback:function(){if(b._isLoaded){b._load()}else{if(b._isReady){b._ready()}}}});this._progressTime.start()},_removeTimer:function(b){b.stop();b=null},_addReadyEvent:function(){document.addEventListener("DOMContentLoaded",this._onDomReady,false)},_addLoadEvent:function(){window.addEventListener("load",this._onWindowLoad,false)},_removeReadyEvent:function(){document.removeEventListener("DOMContentLoaded",this._onDomReady,false)},_removeLoadEvent:function(){window.removeEventListener("load",this._onWindowLoad,false)},pause:function(){this._progressTime.pause()},start:function(){this._progressTime.start()}};a.load51=null;a.Share51=function(b){this._location=location.href;this._basePath=this.getBasePath();this._shareConfig={title:b.title||document.title,desc:b.desc||this._location,link:b.link||this._location,imgUrl:this._basePath+b.imgUrl,trigger:function(c){},success:function(c){},cancel:function(c){},fail:function(c){}};if(!this._shareConfig.imgUrl){throw new Error("分享图片是必须的")}if(!$){throw new ReferenceError("请在这个js文件之前引入jQuery或者Zepto")}this._debug=b.debug||false;this.init()};a.Share51.prototype={constructor:a.Share51,init:function(){this.CreateAjax();this.wxReady();this.wxError();this.addImg()},CreateAjax:function(){$.support.cors=true;var b={url:this._location.split("#")[0]};

var c=+new Date();var d=this;$.ajax({type:"post",url:"http://analysis.51family.com.cn/sh/post.php?time="+c,dataType:"json",data:b,success:function(e){console.log("success");d._xhrSuccess(e)},error:function(e,g,f){d._xhrError();console.log("error")}})},_xhrSuccess:function(b){this._wxConfig(b);if(this._debug){this.showShareDebug()}},_xhrError:function(b){},_wxConfig:function(b){var c=this;wx.config({debug:c._debug,appId:b.appId,timestamp:b.timestamp,nonceStr:b.nonceStr,signature:b.signature,jsApiList:["checkJsApi","onMenuShareTimeline","onMenuShareAppMessage","onMenuShareQQ","onMenuShareWeibo","onMenuShareQZone"]});console.log("_wxConfig")},wxReady:function(){var b=this;wx.ready(function(){wx.onMenuShareAppMessage(b._shareConfig);wx.onMenuShareTimeline(b._shareConfig);wx.onMenuShareQQ(b._shareConfig);wx.onMenuShareWeibo(b._shareConfig);wx.onMenuShareQZone(b._shareConfig);console.log("wxReady")})},wxError:function(){var b=this;wx.error(function(c){console.log("wxError",c)})},getBasePath:function(){var d="",b="",c="";if(b in location){b=location.origin}else{b=location.protocol+"//"+location.hostname+(location.port?":"+location.port:"")}c=location.pathname.split("/");c.pop();c.push("");c=c.join("/");d=b+c;return d},addImg:function(){var b=document.body;if(!b){return}var c=document.createElement("img");c.src=this._shareConfig.imgUrl;c.style.cssText="position:absolute;top:-200%;left:0;width:320px;height:auto";b.insertBefore(c,b.childNodes[0])},showShareDebug:function(){console.log("%c--------------------SHARE START--------------------\n%cShare51\u63d0\u793a\n%c\u5206\u4eab\u6807\u9898\uff1a%c"+this._shareConfig.title+"\n%c\u5206\u4eab\u8be6\u60c5\uff1a%c"+this._shareConfig.desc+"\n%c\u5206\u4eab\u56fe\u7247\uff1a%c"+this._shareConfig.imgUrl+"\n%c\u5206\u4eab\u94fe\u63a5\uff1a%c"+this._shareConfig.link+"\n%c--------------------SHARE END--------------------","color:#989a9d;","font-size:20px;","color:#000;","color:green;","color:#000;","color:green;","color:#000;","color:green;","color:#000;","color:green;","color:#989a9d;")}};a.share51=null;return a})(window.KeirTools||{});function load(b,a){return window.KeirTools.load51||(window.KeirTools.load51=new window.KeirTools.Load51(true,b,a))}function share(a){return window.KeirTools.share51||(window.KeirTools.share51=new window.KeirTools.Share51(a))};	
...
//兼容音乐
var playmusic={changeClass:function(target,id){var className=$(target).attr('class'),ids=document.getElementById(id);(className=='on')?$(target).removeClass('on').addClass('off'):$(target).removeClass('off').addClass('on');(className=='on')?ids.pause():ids.play()},play:function(){document.getElementById('media').play()}};

//$('.footer').html('<img src="images/hand.gif" width="100%" >无忧工作网版权所有&copy;1999-'+(new Date()).getFullYear());function debug(){console.log(arguments[0])}
